...
BIS_crashSite = {
	...
	*/
	...
	[] spawn {
		...
		BIS_offTruck setPos [5690.8989,5313.7993,-0.8];
		BIS_offTruck setDir 125;
...
BIS_campSetup = {
	...
	BIS_logistics setPos [7093.7,5965.62,0.0014205];
...
// Replace map H barrier with new one
private _old = (nearestTerrainObjects [[4997.08,5913.55,1.59297], [], 1, false]) select 0;
private _new = createVehicle ["Land_HBarrier_3_F", [10,10,10], [], 0, "NONE"];
_new setPosATL (getPosATL _old);
_new setVectorDirAndUp [vectorDir _old, vectorUp _old];
_old hideObject true;
_new allowDamage false;

// Disable collisions with officer
{
	BIS_logistics disableCollisionWith _x;
	_x disableCollisionWith BIS_logistics;
} forEach [_new, BIS_invisibleWall1, BIS_invisibleWall2];

// Move logic slightly forward
private _pos = [BIS_logic1, 0.25, 150] call BIS_fnc_relPos;
_pos set [2, 0];
BIS_logic1 setPos _pos;
