...
switch (toLower _action) do
{
	...
	case (tolower "initialize") :
	{
		...
		// Convoy flow
		//private _convoyFlow = [_first, _last, cheatsEnabled] execFSM "fsms\convoyFlow.fsm";
		...
		{
			//private _vehicleFlow = [_vehicles select _i, _vehicles select (_i - 1), _first, cheatsEnabled] execFSM "fsms\vehicleFlow.fsm";
		...
		missionNameSpace setVariable ["BIS_convoyGroup", group driver (_vehicles select 0)];

		// Start with movement disabled
	...
	case (tolower "terminateVehicle") :
	{
		...
		["setSpeed", -1] call BIS_fnc_C_m02_convoy;
	};
	...
	{
		//Parameters
		private _speed = [_this, 1, SPEED_DEFAULT, [0]] call BIS_fnc_param;
		private _vehicles = missionNameSpace getVariable ["BIS_convoyVehicles", []];

		if (count _vehicles > 0) then
		{
			//(_vehicles select 0) limitSpeed _speed;
			(driver (_vehicles select 0)) forceSpeed _speed;
