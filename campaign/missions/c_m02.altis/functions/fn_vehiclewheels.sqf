...
switch (toLower _action) do {
	...
	case (toLower "handleDamage") : {
		...
		if (_source == player && _selectionLowerCase in WHEELS && { _damage >= 0.9 } && { !(_selectionLowerCase in _brokenWheels) }) then {
			//Store broken wheel
			...
			//Log
			["Vehicle (%1) wheel (%2) broken from source %3", _vehicle, _selectionLowerCase, _source] call BIS_fnc_logFormat;
		};
