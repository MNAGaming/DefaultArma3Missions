...
// Handle crew if the first helicopter is disabled
BIS_OP_heli6 addEventHandler [
	"GetOut",
	{
		...
		if (_position != "cargo") then {
			...
			if (leader _unit == _unit) then {
				// Forget remaining waypoints
				private ["_group"];
				_group = group _unit;
				while {count waypoints _group > 0} do {deleteWaypoint (waypoints _group select 0)};
				
				private ["_wp"];
				_wp = _group addWaypoint [position _unit, 0];
