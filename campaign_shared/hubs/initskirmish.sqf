...
BIS_Skirmish_createMap =
{
	...
	[
		...
		2,
		true,
		localize "STR_A3_select_starting_position",
		false/*,
	...
	private _tasks = [];
	{
		private _destination = _x call bis_fnc_taskDestination;
		private _destinationTarget = _destination param [0, objNull];

		["[ ] _destination: %1 | _destinationTarget: %2",_destination,_destinationTarget] call BIS_fnc_logFormat;

		if !(_destinationTarget isEqualType objNull && {isNull _destinationTarget}) then
		{
			_tasks pushBack _x;
			_tasks pushBack _destination;
			[_x, [objNull, false]] call bis_fnc_taskSetDestination;
		};
	}
	...
	//show tasks
	if (count _tasks > 1) then
	{
		for "_i" from 0 to count _tasks - 2 step 2 do
		{
			[_tasks select _i,_tasks select (_i + 1)] call bis_fnc_taskSetDestination;
		};
	};
...
if (true) then
{
	...
	private["_unit","_squad","_added","_squadVars"];
	...
	//setup skirmish characters that are on HUB
