...
objNull,0,nil,true,"whiteboard"] call BIS_fnc_taskCreate;
...
private _iconTemplate = if (count _missions == 1) then {"talk"} else {"talk%1"};

{
	...
	//create task
	[player,[_mission,"BIS_ReportIn"],[_taskDesc,_taskName,_taskMarker],[_npc,true],0,nil,false,format[_iconTemplate,_forEachIndex + 1],true] call BIS_fnc_taskCreate;
	...
	while {BIS_selectedMission == ""} do
	{
		if (true) then
		{
			...
			waitUntil
			{
				...
				{
					//["[ ][FullStart] Mission already selected."] call BIS_fnc_logFormat;
				};
				...
				{
					//["[ ][FullStart] Non-mission task selected!"] call BIS_fnc_logFormat;
				};
			...
			waitUntil
			{
				...
				{
					//["[ ][FullStart] Current task is not pointing to briefing area!"] call BIS_fnc_logFormat;
				};
