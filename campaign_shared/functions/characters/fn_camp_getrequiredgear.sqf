...

{
	if (true) then
	{
		...
		_possessedCount = switch (_type) do
		{
			...
			case "weapon":
			{
				if (_class == "binocular") then
				{
					([_unit,[_type,"binocular"]] call BIS_fnc_camp_getEquipCount) + ([_unit,[_type,"rangefinder"]] call BIS_fnc_camp_getEquipCount) + ([_unit,[_type,"laserdesignator"]] call BIS_fnc_camp_getEquipCount)
				}
				else
				{
