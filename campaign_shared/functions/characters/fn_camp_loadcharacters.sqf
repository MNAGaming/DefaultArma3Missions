...
if (_loadingBriefing) then
{
	[_units,_units call BIS_fnc_camp_poolGetCharacters] spawn
	{
		private["_units","_preBriefingGear","_postBriefingGear"];

		_units = _this select 0;
		_preBriefingGear = _this select 1;

		waitUntil{time > 0};

		_postBriefingGear = _units call BIS_fnc_camp_poolGetCharacters;

		//compare the gear
		private["_type","_typeIndex","_preBriefingData","_postBriefingData","_class","_count"];

		{
			_type			= _x;
			_typeIndex		= _forEachIndex;
			_preBriefingData	= _preBriefingGear select _typeIndex;
			_postBriefingData	= _postBriefingGear select _typeIndex;

			{
				//remove post briefing gear from pre briefing gear to get the difference
				[_preBriefingData,_x select 0,-(_x select 1)] call BIS_fnc_addToPairs;
			}
			forEach _postBriefingData;

			//add the difference to the pool
			{
				_class = _x select 0;
				_count = _x select 1;
				...
				if (_count != 0) then
				{
					//["[x][%1] adding: %3x %2",_type,_class,_count] call BIS_fnc_logFormat;

					[_type,_class,_count] call BIS_fnc_camp_poolAdd;
				};
			}
			forEach _preBriefingData;

			//saving the persistant arrays
			[format["BIS_PER_POOL_%1",_type]] call BIS_fnc_camp_saveVar;
		}
