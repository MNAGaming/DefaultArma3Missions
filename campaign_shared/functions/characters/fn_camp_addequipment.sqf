...
//["[i][%1] Adding [type:%2,class:%3,count:%4,location:%5] ...",_unit,_type,_class,_count,_location] call DEBUG_LOG;
...
else
{
	switch (_type) do
	{
		...
		default
		{
			["[x][Adding failed] unit: %1 | type: %2 | item: %3 | location: %4",_unit,_type,_class,_location] call DEBUG_LOG;
