...
{
	["[x] Hub systems are not multiplayer compatible!"] call BIS_fnc_error;
};
...
if (_isSkirmish) then
{
	//["[!] STARTING LOADSCREEN: %1","bis_campaing_setupSkirmishObjects"] call bis_fnc_logFormat;

	["bis_campaing_setupSkirmishObjects"] call bis_fnc_startLoadingScreen;
};
