...
[_showIntel,_date] spawn
{
	...
	if (_showIntel) then
	{
		...
		{
			if (count _location > 0 && {_location select 0 == ""}) then
			{
				_location deleteAt 0;
			};

			_location = format _location;
		};

		if (typeName _sector == typeName []) then
		{
			if (count _sector > 0 && {_sector select 0 == ""}) then
			{
				_sector deleteAt 0;
			};

			_sector = format _sector;
		};
